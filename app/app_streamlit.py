    ...
    with st.container():
        ...
        if st.session_state.failed_state:
            ...
            if state.current_phase == "initial":
                ...
                completed = len(state.source_text_chunks) + len(
                    state.reflection_chunks
                )
            else:
                completed = len(state.source_text_chunks) * 2 + len(
                    state.translation_2_chunks
                )
            total = len(state.source_text_chunks) * 3
            st.info(
                f"💾 Progress saved: {completed}/{total} API calls completed. Click **Resume** to continue."
            )
...
if st.session_state.results and not st.session_state.translation_started:
    ...
    detail_init.text_area(
        "Initial Translation", value=init, height=150, key="init_prev"
    )
    detail_reflection.text_area(
        "Reflection", value=reflection, height=150, key="reflection_prev"
    )
    ...
    if not source_text:
        ...
        try:
            ...
            try:
                while True:
                    ...
                    elapsed_metric.metric(
                        "⏱️ Elapsed", format_time(elapsed / 60)
                    )

                    if st.session_state.call_times:
                        avg_call_time = sum(st.session_state.call_times) / len(
                            st.session_state.call_times
                        )
                        remaining_calls = event.call_total - event.call_index
                        eta_seconds = remaining_calls * avg_call_time
                        eta_metric.metric(
                            "🎯 ETA", format_time(eta_seconds / 60)
                        )

                        calls_per_min = (
                            60 / avg_call_time if avg_call_time > 0 else 0
                        )
                        speed_metric.metric(
                            "⚡ Speed", f"{calls_per_min:.1f} calls/min"
                        )
                    ...
                    if event.partial_translation:
                        partial_output.text_area(
                            ...
                            value=event.partial_translation[:2000]
                            + (
                                "..."
                                if len(event.partial_translation) > 2000
                                else ""
                            ),
                            height=150,
                            label_visibility="collapsed",
            ...
            if final_result:
                ...
                st.success(
                    f"✅ Translation completed in {format_time(total_time / 60)}!"
                )
